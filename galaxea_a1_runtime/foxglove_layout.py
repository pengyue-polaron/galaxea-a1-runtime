"""Generate tracked Foxglove layout and extension configuration."""
...
COLLECTION_CONSOLE_PANEL_TYPE = "galaxea-a1-collection-console.Collection Console"
...
def render_foxglove_extension_config(system: SystemConfig) -> str:
    """Render the extension's exact topic and service names from System config."""

    services = system.operator_panel.services
    lines = [
        "// Generated by scripts/runtime/render_foxglove_layout.py; do not edit.",
        f"export const WORKFLOW_STATUS_TOPIC = {json.dumps(system.observability.topics.workflow_status)};",
        "export const COLLECTION_SERVICES = {",
    ]
    lines.extend(
        f"  {name}: {json.dumps(value)}," for name, value in services.__dict__.items()
    )
