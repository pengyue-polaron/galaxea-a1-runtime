...
a1_step "Starting hardware-free LingBot training-set replay"
"${LINGBOT_RUNTIME}" server
PYTHONPATH="${ROOT}:${PYTHONPATH:-}" "${PYTHON}" \
  -m galaxea_a1_runtime.apps.eef_policy_offline lingbot \
  --repo-root "${ROOT}" --config "${CONFIG}" --run-id "${RUN_ID}"
"${LINGBOT_RUNTIME}" server-stop

a1_step "Starting hardware-free Pi0.5 training-set replay"
"${PI05_RUNTIME}" server
PYTHONPATH="${ROOT}:${PYTHONPATH:-}" "${PYTHON}" \
  -m galaxea_a1_runtime.apps.eef_policy_offline pi05 \
  --repo-root "${ROOT}" --config "${CONFIG}" --run-id "${RUN_ID}"
"${PI05_RUNTIME}" server-stop

PYTHONPATH="${ROOT}:${PYTHONPATH:-}" "${PYTHON}" \
  -m galaxea_a1_runtime.apps.eef_policy_offline summarize \
  --repo-root "${ROOT}" --config "${CONFIG}" --run-id "${RUN_ID}"

trap - EXIT
cleanup
a1_success "Offline evaluation complete: outputs/offline_evaluation/fruit_placement/${RUN_ID}/REPORT.md"
