...
def normalize_lerobot_tasks(tasks: pd.DataFrame) -> pd.DataFrame:
    tasks = tasks.copy()

    # 已经是 v3 标准格式
    if (
        tasks.index.name == "task_index"
        and "task_name" in tasks.columns
    ):
        return tasks.sort_index()

    # 🔥 核心兜底：index 是 task_name，列里有 task_index
    if "task_index" in tasks.columns:
        # index 一定是 task_name（无论叫不叫这个名字）
        tasks = (
            tasks
            .reset_index()
            .rename(columns={tasks.index.name or "index": "task_name"})
            .set_index("task_index")
        )
        tasks.index.name = "task_index"
        return tasks.sort_index()

    # 如果只有 task_name 列，没有 index
