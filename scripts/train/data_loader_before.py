...
from misc.local_dataset import LocalDataset
...
def create_data_loader(
    ...
    *,
    ...
    elif data_config.local_data_dir is not None:
        print("Using local data loader for A1 dataset")
        return create_local_data_loader(
            data_config,
...
class LocalDataLoader:
    """Local data loader implementation for A1 data."""
    ...
    def __init__(
        ...
        *,
        ...
        if len(dataset) < local_batch_size:
            raise ValueError(
                f"Local batch size ({local_batch_size}) is larger than dataset size ({len(dataset)})."
            )
...
class DataLoaderImpl(DataLoader):
    def __init__(
        self,
        data_config: _config.DataConfig,
        data_loader: TorchDataLoader | RLDSDataLoader | LocalDataLoader,
    ):
...
def create_local_dataset(
    ...
    """Create a local dataset for A1 data."""
...
def create_local_data_loader(
    ...
    *,
    ...
    """Create a local data loader for A1 datasets.
    ...
    Args:
        ...
        action_horizon: The action horizon.
        batch_size: The batch size.
        sharding: The sharding to use for data loader.
        ...
        skip_norm_stats: Whether to skip data normalization.
    """
    dataset = create_local_dataset(data_config, action_horizon)
    dataset = transform_dataset(dataset, data_config, skip_norm_stats=skip_norm_stats)
    ...
    # Create data config factory and instantiate
    data_factory = LeRobotA1DataConfig(repo_id=repo_id)
    data_config = data_factory.create(
        assets_dirs=Path(data_path).parent, model_config=model_config
    )

    # Create the data loader using existing infrastructure
    return create_torch_data_loader(
