...
python run_a1_server.py --mode teleop # which start teleoperating A1
...
cd ../train
uv run compute_norm_stats.py --config-name pi05_localdata_a1_lora
XLA_PYTHON_CLIENT_MEM_FRACTION=0.9 uv run train.py pi05_localdata_a1_lora --exp-name=first_tryout --overwrite
XLA_PYTHON_CLIENT_MEM_FRACTION=0.9 uv run train.py debug --exp-name=debug --overwrite

```

## Inference
```bash
uv run my_serve_policy.py policy:checkpoint \
    --policy.config pi05_localdata_a1_lora \
    --policy.dir /home/jolia/DataCoach/scripts/train/checkpoints/pi05_localdata_a1_lora/first_tryout/29999

# In a new terminal
a1env
cd scripts/collect_data
python run_a1_server.py --mode infer # which start teleoperating A1
