...
def test_lingbot_deployment_composes_with_shared_system_config():
    ...
    assert not config.server.prompt
    ...
    transform = build_action_transform_config(config)
...
def test_lingbot_ready_rejects_missing_real_statistics(tmp_path):
    text = CONFIG.read_text().replace(
        "deployment_ready = false", "deployment_ready = true"
    )
    text = text.replace('prompt = ""', 'prompt = "pick the object"')
    text = text.replace(
        "expected_weight_size_bytes = 0", "expected_weight_size_bytes = 1"
    )
