...
def test_continuous_gripper_mapping_round_trips():
    ...
    assert denormalize_stroke(normalized, stroke_min_mm=0.0, stroke_max_mm=100.0) == pytest.approx(50.0)


def test_continuous_gripper_output_clips_but_feedback_rejects_bad_units():
    assert denormalize_stroke(1.5, stroke_min_mm=0.0, stroke_max_mm=100.0) == 100.0
