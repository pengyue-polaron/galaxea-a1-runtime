...
from a1.training.a1_policy import A1Inputs, A1JointInputs, A1JointOutputs, A1Outputs
...
class DataConfig:
    ...
    datasets: Sequence[droid_rlds_dataset.DroidRldsDataset] = ()
    # Local data directory. If None, data will be loaded from LeRobot.
    # This is useful for loading local datasets that are not stored in LeRobot.
...
@dataclasses.dataclass(frozen=True)
class LocalDataA1DataConfig(DataConfigFactory):
    """
    DataConfig for A1 datasets stored in LeRobot format version 3.0
    ...
    def create(
        ...
        # 1️⃣ Dataset → pipeline key alignment
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
        ...
        # 2️⃣ Semantic transforms (shared train / inference)
        data_transforms = _transforms.Group(
            inputs=[A1Inputs(model_type=model_config.model_type)],
            outputs=[A1Outputs()],
        )

        # 3️⃣ Model-level transforms (tokenization, etc.)
        model_transforms = ModelTransformFactory()(model_config)
        
        assert self.local_data_dir is not None, "Need to set local data dir for LocalDataA1DataConfig."

        # 4️⃣ Assemble full config
        return dataclasses.replace(
...
@dataclasses.dataclass(frozen=True)
class LocalDataA1JointDataConfig(DataConfigFactory):
    """DataConfig for A1 datasets in LeRobot v2.1 format — 7D joint space.

    Dataset keys (parquet columns):
      - cam_0, cam_1 : image paths
      - state        : (7,)  [joint1..joint6, gripper]
      - action       : (7,)  [joint1..joint6, gripper]  (= state[t+1])
    """
    ...
    def create(
        ...
        # Our dataset stores absolute joint targets (action[t] = state[t+1]).
        # Pi0/Pi0.5 base models are trained on delta actions (relative to current state).
        # Apply delta conversion to the 6 joint dims; keep gripper (dim 7) absolute.
        delta_action_mask = _transforms.make_bool_mask(6, -1)
        data_transforms = data_transforms.push(
            inputs=[_transforms.DeltaActions(delta_action_mask)],
            outputs=[_transforms.AbsoluteActions(delta_action_mask)],
        ...
        assert self.local_data_dir is not None, "Set local_data_dir for LocalDataA1JointDataConfig."
